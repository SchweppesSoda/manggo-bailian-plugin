...
test("manifest declares a v2.3 plugin with both services", () => {
...
test("services expose all billing modes and default to pay-as-you-go", () => {
...
test("translation and OCR presets include plan and pay-as-you-go models", () => {
  ...
  assert.ok(translationModels.includes("qwen3.6-plus"));
  assert.ok(translationModels.includes("glm-5"));
  ...
  assert.ok(ocrModels.includes("qwen3.6-plus"));
  assert.ok(ocrModels.includes("kimi-k2.5"));
...
test("repository includes license, security policy, CI, and single-profile packaging", async () => {
  ...
  assert.doesNotMatch(packageScript, /PersonalCoding|Set-RuntimeProfile|CODING-PLAN-NOTICE/);
  assert.match(packageScript, /verify-package\.mjs/);
});
