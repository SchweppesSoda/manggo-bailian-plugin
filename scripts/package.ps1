...
$packageVerifier = Join-Path $repoRoot 'scripts/verify-package.mjs'
& node $packageVerifier $manggoStage (Join-Path $stagingRoot 'bob-translate') (Join-Path $stagingRoot 'bob-ocr')
if ($LASTEXITCODE -ne 0) {
    throw 'Runtime package verification failed.'
}
...
function Assert-ArchiveModes {
    ...
    try {
        foreach ($entry in $archive.Entries | Where-Object { $_.FullName -match '\.js$' }) {
            ...
            if ($content -match '__BAILIAN_RUNTIME_PROFILE_(DEVELOPMENT|PERSONAL|PUBLIC)__') {
                throw "$(Split-Path -Leaf $ArchivePath) contains an obsolete runtime profile marker."
            }
        }
        ...
        if (-not $hasCodingOption -or -not $hasTokenOption) {
            throw "$(Split-Path -Leaf $ArchivePath) is missing a subscription-plan option."
        }
    }
    finally {
        $archive.Dispose()
...
Assert-ArchiveModes -ArchivePath $manggoPackage -ManifestEntry 'manggo.plugin.json'
Assert-ArchiveModes -ArchivePath $translatePackage -ManifestEntry 'info.json'
Assert-ArchiveModes -ArchivePath $ocrPackage -ManifestEntry 'info.json'
