...
// src/manggo/entry.js
function configFrom2(options) {
  return options && options.config ? options.config : {};
}
async function translate(text, from, to, options) {
  const input = String(text === void 0 || text === null ? "" : text);
  if (!input) return "";
  const detected = options && options.detect;
  return complete(translationRequest(input, from, to, configFrom2(options), detected), options, false);
}
async function recognize(base64, language, options) {
  return complete(ocrRequest(base64, language, configFrom2(options)), options, true);
}
export {
  recognize,
  translate
